...
bool incircle(float x, float y, float r, float co1, float co2) {
	return (co1-x)*(co1-x) + (co2-y)*(co2-y) <= r*r;
}
...
int main(int argc, char* argv[]) {
	...
	while(i < argc){
		if(!strcmp(argv[i], "-i") && !in){
			...
			offset = atoi(argv[i+1]) * 2*sizeof(float); //testing
			i += 2;
			ofs = true;
		}
	...
	if(!nump){
		while(fread(coord, sizeof(float), 2, input)){
			if(incircle(x, y, r, coord[0], coord[1])){
				sprintf(line, "%f\t%f\n", coord[0], coord[1]);
