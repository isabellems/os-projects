...
bool inring(float x, float y, float r1, float r2 , float co1, float co2){
	return (((co1-x)*(co1-x) + (co2-y)*(co2-y) <= r1*r1) && ((co1-x)*(co1-x) + (co2-y)*(co2-y) >= r2*r2));
}
...
int main(int argc, char* argv[]) {
	...
	if(!nump){
		while(fread(coord, sizeof(float), 2, input)){
			if(inring(x, y, r1, r2, coord[0], coord[1])){
				sprintf(line, "%f\t%f\n", coord[0], coord[1]);
